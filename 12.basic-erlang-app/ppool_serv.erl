...
%% API
-export([start/4, init/1, start_link/4, run/2, sync_queue/2, async_queue/2, stop/1]).
...
run(Name, Args) ->
    gen_server:call(Name, {run, Args}, infinity).

sync_queue(Name, Args) ->
    gen_server:call(Name, {sync, Args}, infinity).

async_queue(Name, Args) ->
    gen_server:cast(Name, {async, Args}, infinity).
...
handle_call({run, Args}, _From, S = #state{limit=N, sup=Sup, refs=R}) when N > 0->
    io:format("Args received: ~p~n", [Args]),
    io:format("State received: ~p~n", [S]),
